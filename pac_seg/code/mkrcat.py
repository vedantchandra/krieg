...
if hostname[:4] == 'holy':
    ...
    outdir  = '/n/holystore01/LABS/conroy_lab/Lab/SEGUE/'
else:
    outdir  = '/Users/pcargile/Astro/SEGUE/'

class mkrcat(object):
    """docstring for mkrcat"""
    def __init__(self, acatfile, version, runtype, outfile):
        super(mkrcat, self).__init__()
        self.acatfile = acatfile
    ...
    def filltab(self,rcat_i):
        plateID = rcat_i['PLATE']
        fibID   = rcat_i['FIBERID']
        mjd     = rcat_i['MJD']
        gaiaID  = rcat_i['GAIAEDR3_ID']

        parfile = '{OUTDIR}/results/{VER}/pars/segue_gaiaID_{GAIAID}_fibID_{FIBID}_plate_{PLATEID}_mjd_{MJD}_{VER}_{RUNTYPE}.pars'.format(
            OUTDIR=outdir,
            FIBID=fibID,
            GAIAID=gaiaID,
            PLATEID=plateID,
    ...
    def run(self):
        # copy acat to rcat
        acat = Table.read(self.acatfile,format='fits')
        rcat = acat.copy()
        del acat
        ...
        if self.outfile is None:            
            outfile = "{OUTDIR}/catalogs/SEGUE_rcat_{VERSION}_{RUNTYPE}.fits".format(
                OUTDIR=outdir,
...
if __name__ == '__main__':
    ...
    parser.add_argument("--acatfile", "-a", help="which acat to use",type=str)
    parser.add_argument("--version", "-v", help="run version",type=str,default='V0')
    ...
    MK = mkrcat(args.acatfile,args.version,args.runtype,args.outfile)
    MK.run()
