...
def run(catalog=None,index=None,version='VX', npoints = 250, skipfit = 0):
    ...
    if skipfit == 0:
        ...
        raise
    sys.stdout.flush()
    compmod.run(index = index)
    sys.stdout.flush()
    corner.run(index = index)
    sys.stdout.flush()
    calcpars.run(index = index)
    sys.stdout.flush()
...
if __name__ == '__main__':
    ...
    parser.add_argument('--index',help='Index of star',type=int,default=None)
    ...
    run(catalog=args.catalog,index=args.index,version=args.version,npoints=args.npoints,
