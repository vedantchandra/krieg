...
outdir  = '/n/holyscratch01/conroy_lab/vchandra/segue/'
...
class mkrcat(object):
    ...
    def filltab(self,rcat_i):
        catID = rcat_i['GAIAEDR3_ID']
        fieldID   = rcat_i['FIBERID']
        mjd     = rcat_i['MJD']
        gaiaID  = rcat_i['GAIAEDR3_ID']

        parfile = '{OUTDIR}pars/{CATALOG}/{VER}/mwm_gaiaID_{GAIAID}_fieldID_{FIELDID}_mjd_{MJD}_catID_{CATID}_{VER}.pars'.format(
            OUTDIR=outdir,
            FIELDID=fieldID,
            GAIAID=gaiaID,
            CATID=catID,
    ...
    def run(self):
        # copy acat to rcat
        acat = Table.read(outdir + 'data/SEGUEtestspec_acat.fits')
	#acat = Table.read(outdir + 'catalogs/%s_acat.fits' % self.catalog,format='fits')
        rcat = acat.copy()
        del acat

        for ii,col in enumerate(list(rcat.columns)):
            if ii >= 119:
                rcat.rename_column(col, 'pac_' + col)
