...
datadir = '/n/holyscratch01/conroy_lab/vchandra/segue/'
...
def run(index=None,GaiaID=None,version='VX', npoints = 250,catalog = None,
    ...
    datfiles = glob.glob(datadir + 'data/*GaiaDR*.fits')
    acatfile = datadir + 'data/SEGUEtestspec_acat.fits'
    acat = Table.read(acatfile)
    catalog = 'test'

    star = acat[index]
    for file in datfiles:
        if str(star['GAIAEDR3_ID']) in file:
            specfile = file 
    ...
    data['spec']['wave'] = spec['wave']
    data['spec']['flux'] = spec['flux']
    data['spec']['eflux'] = spec['eflux']
    data['spec']['wresl'] = spec['wresl']

    samplefile = 'mwm_gaiaID_{GAIAID}_fieldID_{FIELDID}_mjd_{MJD}_catID_{CATID}_{VER}_samp.dat'.format(
            FIELDID=data['phot']['FIBERID'],
            GAIAID=data['phot']['GAIAEDR3_ID'],
            CATID=data['phot']['GAIAEDR3_ID'],
            MJD=data['phot']['MJD'],
            VER=version)
    ...
    print('-- Running survey={0} field={1} mjd={2}'.format('MWM',data['phot']['FIBERID'],data['phot']['MJD']))
    print('   ... GaiaEDR3 ID = {0}'.format(data['phot']['GAIAEDR3_ID']))
    print('   ... SDSS5 CATALOG ID = {0}'.format(data['phot']['GAIAEDR3_ID']))
    ...
    spec['IVAR']   = 1/specdata['eflux']**2
    spec['E_FLUX'] = specdata['eflux']
